"""Correctness test for the full v3-q3a batched-MHA forward.

Contract: kernel_function(*inputs) must match the eager reference Model on
the real serving shapes within bf16 tolerances. Floating inputs are cast to
bf16 on cuda; bool masks move to cuda unchanged. Both sides return one
(F, B, Lq, E) tensor.
"""
...
# Reference does fp32 matmuls mirroring the kernels' ieee fp32 accumulate.
